prompt_command_timer_preexec() {
   prompt_timer_start_ms=$(( $(printf "%.0f" "${EPOCHREALTIME} * 1000") ))
   prompt_elapsed_time=''
}

prompt_command_timer_precmd() {
...
PROMPT='%F{10}%n%f'"${prompt_host}"':%F{14}%~%f$(gitprompt)${prompt_elapsed_time}'$'\n''❯ '

add-zsh-hook preexec prompt_command_timer_preexec
add-zsh-hook precmd prompt_command_timer_precmd
